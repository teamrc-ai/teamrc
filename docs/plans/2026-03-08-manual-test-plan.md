...
> **Known issue:** `section-03-platforms.sh` OpenClaw checks reference the old workspace model (`~/.openclaw/workspaces/trc-*/`). OpenClaw now uses file-based agents at `~/.openclaw/agents/trc-*.md`. Update the script before running OpenClaw verification.
...
```bash
npx @teamrc/cli whoami
npx @teamrc/cli whoami --json
```

- [ ] Shows machine token (truncated), machine name, team ID, relay URL, platforms
- [ ] If account linked: shows email
- [ ] If no account linked: no email field
...
- [ ] `openclaw.json` preserves non-`trc-` agent entries
- [ ] First `trc-*` agent gets `default: true` if no existing default
- [ ] No legacy `~/.openclaw/workspaces/trc-*` directories created
- [ ] `teamrc status` shows agent files in global scope
...
**A:** `npx @teamrc/cli daemon --poll-interval 5000`
...
Navigate to `/`:
- [ ] Marketing/landing page loads (HomeLive)
- [ ] Terminal quickstart block visible with CLI commands
- [ ] Platform logos/badges displayed
- [ ] Link to `/new` (create team) works
- [ ] Link to `/guide` works
...
**Note:** OpenClaw is global-only. Agent files are at `~/.openclaw/agents/trc-*.md` (file-based, not workspace-based). Skills are at `~/.openclaw/skills/trc-*/SKILL.md`. The `openclaw.json` config file at `~/.openclaw/openclaw.json` registers agents.
