...
The `docker-compose.coolify.yml` file deploys the Phoenix app only. It connects to the Postgres database you created in Step 5. Coolify auto-generates all secrets.
...
These are stable across redeploys. You can view or override them in the **Environment Variables** tab.

**Optional**: for GitHub or Google sign-in on the web dashboard:
...
The app and database must be on the same Docker network. The compose file joins the `app` service to the `coolify` external network, but you also need to connect your Postgres resource to this network.

**For the app** (automatic): the compose file includes `networks: coolify: external: true`, which joins the app to the `coolify` network.

**For the database** (manual step): go to your `teamrc-db` resource in Coolify, then **Settings**, and enable **"Connect to Predefined Network"**. This puts Postgres on the same `coolify` network so the app can reach it.

> **Note**: The "Connect to Predefined Network" toggle is unreliable for Docker Compose resources, which is why the compose file handles it explicitly. For standalone database resources, the toggle works fine.
...
**Fix**: Check the **Environment Variables** tab in Coolify. The `SERVICE_PASSWORD_*` variables should be auto-generated on first deploy. If missing, try redeploying. If still missing, generate them manually with `openssl rand -base64 64 | tr -d '\n'`.
