defmodule Teamrc.AuthTest do
  ...
  describe "to_token/1" do
    test "creates a token with trc_ak_ prefix", %{pub: pub} do
      token = Auth.to_token(pub)
      assert String.starts_with?(token, "trc_ak_")
    end
  ...
  describe "from_token/1" do
    ...
    test "returns error for invalid base64 in token" do
      assert {:error, :invalid_token} = Auth.from_token("trc_ak_!!!invalid!!!")
    end
  end
