defmodule Teambridge.Accounts do
  @moduledoc "Context module for account and machine token operations."

  alias Teambridge.Repo
  alias Teambridge.Schema.{Account, AccountToken, TokenTeam}
  import Ecto.Query
  ...
  def get_account_teams(account_id) do
    from(t in Teambridge.Schema.Team,
  ...
  def revoke_token(account_id, token) do
    case Repo.get_by(AccountToken, account_id: account_id, token: token) do
      ...
      account_token ->
        ...
        GenServer.cast(Teambridge.Teams, {:token_revoked, token})
