defmodule TeambridgeWeb.AccountControllerTest do
  use TeambridgeWeb.ConnCase, async: false

  import Ecto.Query
  alias Teambridge.Repo
  alias Teambridge.Schema.{Account, AccountToken, Team, Member, TokenTeam}
  ...
  describe "GET /api/account" do
    test "returns account and machines", %{conn: conn, account: account} do
      ...
      assert machine["token"] == "tb_ak_machin..."
    end
  ...
  describe "DELETE /api/account/machines/:token" do
    ...
    test "returns 404 for token not owned by caller", %{conn: conn} do
      conn = delete(conn, "/api/account/machines/tb_ak_someone_else")
    ...
    test "returns error for unknown token", %{conn: conn} do
      conn =
        ...
        |> post("/api/account/reassociate", %{
          "new_token" => "tb_ak_unknown_token",
          "token" => "tb_ak_unknown_token"
        })
