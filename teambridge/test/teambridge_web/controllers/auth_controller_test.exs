defmodule TeambridgeWeb.AuthControllerTest do
  ...
  describe "GET /api/auth/device/:device_code" do
    ...
    test "returns confirmed status after confirmation", %{conn: conn} do
      ...
      Teambridge.DeviceAuth.confirm_request(user_code, "clerk_test_123", "test@example.com")
    ...
    test "returns 403 for token mismatch", %{conn: conn} do
      ...
      conn = get(conn, "/api/auth/device/#{device_code}", %{"token" => "tb_ak_wrong_token"})
