defmodule Teambridge.DeviceAuthTest do
  ...
  describe "create_request/2" do
    test "returns valid device_code and user_code", %{pid: pid} do
      token = "tb_ak_test_#{:erlang.unique_integer([:positive])}"
  ...
  describe "poll_request/3" do
    ...
    test "token mismatch returns error", %{pid: pid} do
      ...
      assert {:error, :token_mismatch} =
               DeviceAuth.poll_request(pid, result.device_code, "tb_ak_other_token")
    end

    test "not found returns error", %{pid: pid} do
      assert {:error, :not_found} =
               DeviceAuth.poll_request(pid, "nonexistent_code", "tb_ak_whatever")
    end
  end
  ...
  describe "rate limiting" do
    ...
    test "different tokens can each have 3 requests", %{pid: pid} do
      token1 = "tb_ak_test_#{:erlang.unique_integer([:positive])}"
      token2 = "tb_ak_test_#{:erlang.unique_integer([:positive])}"
